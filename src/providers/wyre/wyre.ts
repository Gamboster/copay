...
export class WyreProvider {
  ...
  constructor(
    ...
    this.logger.debug('WyreProvider Provider initialized');
    this.credentials = {};
  }
  ...
  public setCredentials() {
    ...
    if (
      ...
      !this.appProvider.servicesInfo.wyre
    ) {
      return;
    }
    const wyre = this.appProvider.servicesInfo.wyre;
    console.log("----------service info wyre: ", wyre);
    ...
    this.credentials.HOST = wyre[this.credentials.ENV].host;
    this.credentials.API = wyre[this.credentials.ENV].api;
  }
  ...
  public shift(data, cb) {
    ...
    this.httpNative.post(url, dataSrc, headers).subscribe(
      data => {
        this.logger.info('wyre SHIFT: SUCCESS');
        return cb(null, data);
      },
      data => {
        const error = this.parseError(data);
        this.logger.error('wyre SHIFT ERROR: ' + error);
        return cb(error);
      }
    );
  }
  ...
  public sendamount(data, cb) {
    ...
    this.httpNative.post(url, dataSrc, headers).subscribe(
      data => {
        this.logger.info('wyre SENDAMOUNT: SUCCESS');
        return cb(data.error, data.success);
      },
      data => {
        const error = this.parseError(data);
        this.logger.error('wyre SENDAMOUNT ERROR: ' + error);
        return cb(error);
      }
    );
  }

  public saveWyre(data, opts, cb): void {
    ...
    this.persistenceProvider
      ...
      .then(oldData => {
        ...
        this.persistenceProvider.setWyre(network, inv);
        return cb(null);
      })
      .catch(err => {
        return cb(err);
  ...
  public getRate(pair: string, cb) {
    this.httpNative.get(this.credentials.API_URL + '/rate/' + pair).subscribe(
      data => {
        this.logger.info('Wyre PAIR: SUCCESS');
        return cb(null, data);
      },
      data => {
        const error = this.parseError(data);
        this.logger.error('Wyre PAIR ERROR: ' + error);
        return cb(data);
      }
    );
  }
  ...
  public getLimit(pair: string, cb) {
    this.httpNative.get(this.credentials.API_URL + '/limit/' + pair).subscribe(
      data => {
        this.logger.info('Wyre LIMIT: SUCCESS');
        return cb(null, data);
      },
      data => {
        const error = this.parseError(data);
        this.logger.error('Wyre LIMIT ERROR: ' + error);
        return cb(data);
      }
    );
  }
  ...
  public getMarketInfo(pair: string, cb) {
    this.httpNative
      ...
      .subscribe(
      data => {
        this.logger.info('Wyre MARKET INFO: SUCCESS');
        return cb(null, data);
      },
      data => {
        const error = this.parseError(data);
        this.logger.error('Wyre MARKET INFO ERROR: ', error);
        return cb(data);
      }
      );
  }
  ...
  public getStatus(addr: string, token: string, cb) {
    ...
    this.httpNative
      ...
      .subscribe(
      data => {
        this.logger.info('Wyre STATUS: SUCCESS');
        return cb(null, data);
      },
      data => {
        const error = this.parseError(data);
        this.logger.error('Wyre STATUS ERROR: ' + error);
  ...
  private isActive(cb) {
    ...
    this.persistenceProvider
      .getWyreToken(this.credentials.NETWORK)
      .then(accessToken => {
  ...
  public register(): void {
    this.isActive(isActive => {
      this.homeIntegrationsProvider.register({
        name: 'wyre',
        title: 'Wyre',
        icon: 'assets/img/wyre/icon-wyre.svg',
        logo: 'assets/img/wyre/logo-wyre-light.png',
        background:
          'linear-gradient(to bottom,rgba(54, 90, 202,1) 0,rgba(55, 89, 204,1) 100%)',
        page: 'WyrePage',
        show: !!this.configProvider.get().showIntegration['wyre'],
        linked: isActive,
        logoWidht: 110
      });
    });
  ...
  private _afterTokenReceived(data, cb) {
    if (data && data.access_token) {
      this.persistenceProvider.setWyreToken(
        this.credentials.NETWORK,
        data.access_token
      );
      this.homeIntegrationsProvider.updateLink('wyre', data.access_token); // Name, Token
  ...
  public init = _.throttle(cb => {
    ...
    this.getStoredToken(accessToken => {
      if (!accessToken) {
        this.logger.debug('Wyre not linked');
        return cb();
      }
      this.logger.debug('Wyre already has Token.');
  ...
  private getAccessTokenDetails(token, cb) {
    ...
    this.httpNative.get(url, null, headers).subscribe(
      data => {
        this.logger.info('Wyre: Get Access Token Details SUCCESS');
        return cb(null, data);
      },
      data => {
        const error = this.parseError(data);
        this.logger.error(
          'Wyre: Get Access Token Details ERROR ' +
          data.status +
          '. ' +
          error
        );
  ...
  public getAccount(token, cb) {
    ...
    this.httpNative.get(url, null, headers).subscribe(
      data => {
        this.logger.info('Wyre: Get Account SUCCESS');
        return cb(null, data);
      },
      data => {
        const error = this.parseError(data);
        this.logger.error(
          'Wyre: Get Account ERROR ' + data.status + '. ' + error
        );
        return cb(data.error);
      }
    );
  ...
  public revokeAccessToken(token) {
    ...
    this.httpNative.post(url, data, headers).subscribe(
      () => {
        this.logger.info('Wyre: Revoke Access Token SUCCESS');
      },
      data => {
        const error = this.parseError(data);
        this.logger.warn(
          'Wyre: Revoke Access Token ERROR ' + data.status + '. ' + error
        );
      }
    );
  }
  ...
  public logout(token) {
    ...
    this.persistenceProvider.removeWyreToken(this.credentials.NETWORK);
    this.homeIntegrationsProvider.updateLink('wyre', null); // Name, Token
  }
